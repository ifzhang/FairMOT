...
def write_results_reid(filename, results, data_type):
    # if data_type == 'mot':
    #     save_format = '{frame},{id},{x1},{y1},{w},{h},{reid}\n'
    # elif data_type == 'kitti':
    #     save_format = '{frame} {id} pedestrian 0 0 -10 {x1} {y1} {x2} {y2} -10 -10 -10 -1000 -1000 -1000 -10\n'
    # else:
    #     raise ValueError(data_type)
    ...
    with open(filename, 'w') as f:
        for frame_id, tlwhs, track_ids, reids in results:
            if data_type == 'kitti':
                frame_id -= 1
            for tlwh, track_id, reid in zip(tlwhs, track_ids, reids):
                if track_id < 0:
                    continue
                x1, y1, w, h = tlwh
                x2, y2 = x1 + w, y1 + h
                ...
                # line = save_format.format(frame=frame_id, id=track_id, x1=x1, y1=y1, x2=x2, y2=y2, w=w, h=h, reid=reid.tolist())
                json.dump(d, f)
    ...
    for i, (path, img, img0) in enumerate(dataloader):
        ...
        for t in online_targets:
            ...
            if tlwh[2] * tlwh[3] > opt.min_box_area and not vertical:
                ...
                online_reids.append(t.curr_feat)
...
def main(opt, data_root, seqs, exp_name: str, result_root: str,
...
if __name__ == '__main__':
    ...
    if opt.test_mot17:
        ...
        seqs = [p.split("/")[-1] for p in glob.glob(os.path.join(data_root, "MOT*-SDP"))]
    ...
    if opt.val_mot20:
        ...
        seqs = [p.split("/")[-1] for p in glob.glob(os.path.join(data_root, "MOT*"))]
    ...
    main(opt,
         ...
         seqs=seqs,
         exp_name=opt.exp_id,
         result_root=os.path.join(opt.data_dir, 'results', opt.exp_id),
         show_image=False,
         save_images=False,
         save_videos=False)
